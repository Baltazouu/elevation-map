...
def generate_gpx(route_segments):
    gpx = gpxpy.gpx.GPX()
    ...
    for segment in route_segments:
        for point in segment:
            gpx_point = gpxpy.gpx.GPXTrackPoint(point[0], point[1], elevation=point[2])
            gpx_segment.points.append(gpx_point)

    return gpx.to_xml()  # Retourne le contenu GPX sous forme de texte
